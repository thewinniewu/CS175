...
static float g_xScale = 1.0;
static float g_yScale = 1.0;
...
static void drawSquare() {
  ...
  safe_glUniform1f(g_squareShaderState->h_uXScale, g_xScale);
    safe_glUniform1f(g_squareShaderState->h_uYScale, g_yScale);
  ...
  glDrawArrays(GL_TRIANGLES, 0, 6);
       cout << "\nTEST ";
	   cout << ( (g_squareShaderState->h_aPosition));
...
static void reshape(int w, int h) {
	if ((float)w < (float)h) { // limited by width. set x first
	  float xratio = (float)g_width / (float)w;
									 cout << g_xScale;
									  cout << "higherlalalalala\n";
	  g_xScale *= xratio;
								  cout << g_xScale;
								 cout << "\n";
									cout << g_yScale;
									  cout << "\n";
	  g_yScale = (g_xScale * g_width) / g_height;
	  						   cout << g_yScale;
								 cout << "higherlalalalala\n";

  }
    else { //limited by height. set y first.
	  float yratio = (float)h / (float)g_height;
	    cout << g_yScale;
	  cout << "LOWERLALALALALA\n";
      g_yScale *= yratio;
	    cout << g_yScale;
	  cout << "\n";
	   cout << g_xScale;
	  cout << "\n";
      g_xScale = (g_yScale * g_height) / g_width;
	  	  	   cout << g_xScale;
	  cout << "LOWERLALALALALA\n";
...
static void loadSquareShader(SquareShaderState& ss) {
  ...
  ss.h_uXScale = safe_glGetUniformLocation(h, "uXScale");
  ss.h_uYScale = safe_glGetUniformLocation(h, "uYScale");
  ...
  cout << "\n \n MEEP: ";
       cout << ((vector<float>*) (ss.h_aPosition));
cout << "\n\n ";


  ss.h_xPos = safe_glGetAttribLocation(h, "xPos");
  ss.h_yPos = safe_glGetAttribLocation(h, "yPos");
  ...
  //printf("position: %d", ss.h_aPosition);
 // printf("texcoord: %s", ss.h_aTexCoord);
